...
##
# @package src.GUI
# package with GUI calculator implemented in PyQt6 framework using src.CalcUtils package.
# 
#
# @package src.GUI.calc
# calc.py contains mostly autogenerated code using Qt Designer and pyuic6 with the addition of hadler fucntions for button and keyboard presses
#
...
##
# @class Ui_MainWindow
# QtWidgets.QMainWindow child class
#
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        """!@brief Function us mostly autogenerated from QT Designer
        @param MainWindow QMainWindow object to setup
        @return Nothing
        """
        ...
        #   click events for number buttons assignment
        ...
        #   click events for operators assignement
        ...
        #   click events for text formatting assignment
    ...
    def retranslateUi(self, MainWindow):
        """!@brief Function sets the text and titles of the widgets
    ...
    def click_event_number(self):
        """!@brief Event handler for number buttons
        @return Nothing
        """
        _translate = QtCore.QCoreApplication.translate
    ...
    def click_event_op(self):
        """!@brief Event handler for operator buttons
        @return Nothing
        """
        ops = {
            'plus': '+',
        ...
        btn_val = ops[btn_name.split("_")[1]]   #   get operator value by its name from 'ops' dictionary
    ...
    def click_event_text_format(self):
        """!@brief Event handler for 'AC' and 'DEL' buttons
        @return Nothing
        """
        _translate = QtCore.QCoreApplication.translate
        data = self.lineEdit.text()
    ...
    def click_event_calc(self):
        """!@brief Event handler for calculation action, actual parsing and calculations are called from here
        @return Nothing
        """
        _translate = QtCore.QCoreApplication.translate
        #   scan error was detected, clear input
        if self.scan_error:
            data = ''
        else:
            data = self.lineEdit.text()
        ...
        #   exception handling raised by Parser class
        except SyntaxError:
            ans = 'Syntax error'
        ...
        if not self.scan_error:
            #   no scan error, change sqrt button to 2√x
            self.btn_sqrt.setText(_translate("MainWindow", "2√x"))

    #   calculate when enter/return pressed
    def keyPressEvent(self, event):
        """!@brief Event handler for keyPressEvents from keyboard
...
def format_data(ans):
    """!@brief Helper function that formarts data to int/float, depending on calculation result, and then corverts result to string
    @param ans: Calculated value from Parser class
...
def excepthook(exc_type, exc_value, exc_tb):
    """!@brief Function defines a custom exception handler that prints the traceback of an exception to the standard error output.
    @param exc_type The type of the exception that was raised (e.g. ValueError, TypeError, etc.).
    @param exc_value The exc_value parameter in the excepthook function refers to the value associated with the exception
    that was raised
    @param exc_tb Traceback object that represents the call stack at the point where an exception was raised
    @return Nothing
    """
